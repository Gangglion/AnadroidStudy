...
class OpenSourceCircularProgressBar (context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private val indeterminateModeRunnable = Runnable {
        if (indeterminateMode) {
            ...
            this@OpenSourceCircularProgressBar.progressDirectionIndeterminateMode = this@OpenSourceCircularProgressBar.progressDirectionIndeterminateMode.reverse()
            if (this@OpenSourceCircularProgressBar.progressDirectionIndeterminateMode.isToRight()) {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        val attributes = context.theme.obtainStyledAttributes(attrs, R.styleable.OpenSourceCircularProgressBar, 0, 0)

        // Value
        progress = attributes.getFloat(R.styleable.OpenSourceCircularProgressBar_cpb_progress, progress)
        progressMax = attributes.getFloat(R.styleable.OpenSourceCircularProgressBar_cpb_progress_max, progressMax)

        // StrokeWidth
        progressBarWidth = attributes.getDimension(R.styleable.OpenSourceCircularProgressBar_cpb_progressbar_width, progressBarWidth).pxToDp()
        backgroundProgressBarWidth = attributes.getDimension(R.styleable.OpenSourceCircularProgressBar_cpb_background_progressbar_width, backgroundProgressBarWidth).pxToDp()

        // Color
        progressBarColor = attributes.getInt(R.styleable.OpenSourceCircularProgressBar_cpb_progressbar_color, progressBarColor)
        attributes.getColor(R.styleable.OpenSourceCircularProgressBar_cpb_progressbar_color_start, 0)
            .also { if (it != 0) progressBarColorStart = it }
        attributes.getColor(R.styleable.OpenSourceCircularProgressBar_cpb_progressbar_color_end, 0)
            .also { if (it != 0) progressBarColorEnd = it }
        progressBarColorDirection = attributes.getInteger(R.styleable.OpenSourceCircularProgressBar_cpb_progressbar_color_direction, progressBarColorDirection.value).toGradientDirection()
        backgroundProgressBarColor = attributes.getInt(R.styleable.OpenSourceCircularProgressBar_cpb_background_progressbar_color, backgroundProgressBarColor)
        attributes.getColor(R.styleable.OpenSourceCircularProgressBar_cpb_background_progressbar_color_start, 0)
            .also { if (it != 0) backgroundProgressBarColorStart = it }
        attributes.getColor(R.styleable.OpenSourceCircularProgressBar_cpb_background_progressbar_color_end, 0)
            .also { if (it != 0) backgroundProgressBarColorEnd = it }
        backgroundProgressBarColorDirection = attributes.getInteger(R.styleable.OpenSourceCircularProgressBar_cpb_background_progressbar_color_direction, backgroundProgressBarColorDirection.value).toGradientDirection()

        // Progress Direction
        progressDirection = attributes.getInteger(R.styleable.OpenSourceCircularProgressBar_cpb_progress_direction, progressDirection.value).toProgressDirection()

        // Round Border
        roundBorder = attributes.getBoolean(R.styleable.OpenSourceCircularProgressBar_cpb_round_border, roundBorder)

        // Angle
        startAngle = attributes.getFloat(R.styleable.OpenSourceCircularProgressBar_cpb_start_angle, 0f)

        // Indeterminate Mode
        indeterminateMode = attributes.getBoolean(R.styleable.OpenSourceCircularProgressBar_cpb_indeterminate_mode, indeterminateMode)
