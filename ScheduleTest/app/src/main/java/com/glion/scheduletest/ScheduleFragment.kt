...
class ScheduleFragment : Fragment(), GridItemInterface {
    ...
    override fun onCreateView(
        ...
        // MEMO : 추가 예시
        mScheduleGridLayout.apply{
            addSchedule("치료 일정", "07:00", "09:00")
            changeBackground(this.findItemWithContentName("치료 일정")!!, R.color.blue)
            addSchedule("회고활동 일정", "11:00", "15:00")
            changeBackground(this.findItemWithContentName("회고활동 일정")!!, R.color.red)
            addSchedule("개인 일정", "23:00", "23:30")
            changeBackground(this.findItemWithContentName("개인 일정")!!, R.color.purple)
        }
        ...
        Handler(Looper.getMainLooper()).postDelayed({
            mScheduleGridLayout.apply{
                updateSchedule("치료 일정", "치료 일정_수정", "10:00", "11:00")
                changeBackground(mScheduleGridLayout.findItemWithContentName("치료 일정_수정")!!, R.color.blue)
            }
        }, 2000)
        return view
    }
    ...
    private fun scrollToNowTime(@DrawableRes background: Int? = null) {
        // 현재 시간 계산
        val formatter = DateTimeFormatter.ofPattern("HH:00")
        val nowTime = LocalDateTime.now().format(formatter)
        val item = mScheduleGridLayout.findItem(nowTime, true)
        if(item != null){ // 현재 시간에 해당하는 gridItem 이 없다면 스크롤하지 않음
            val itemRect = mScheduleGridLayout.calculateRectOnScreen(item)
            val y = abs(itemRect.top - mScheduleGridLayout.calculateRectOnScreen(mScrollView).top) / 2
            mScrollView.smoothScrollTo(0, y)
        }
        if(background != null)
            mScheduleGridLayout.findItem(nowTime, false)!!.background = AppCompatResources.getDrawable(mContext, background)
    }
}